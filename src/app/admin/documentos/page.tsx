...
export default function DocumentsPage() {
    const { isElectron, listScannedDocuments, openExternal, renameDocument } = useElectron();
    ...
    const handleLinkToAsset = async () => {
        ...
        for (const path of selectedPaths) {
            ...
            // Strategy: Rename to [CODE] OriginalName
            // Check if already linked to avoid double tagging
            if (doc.name.startsWith(`[${assetCode}]`)) continue;

            const newName = `[${assetCode}] ${doc.name}`;
            const res = await renameDocument(doc.path, newName);
            if (res.success) {
                successCount++;
            } else {
                console.error(`Falha ao renomear ${doc.name}: ${res.error}`);
            }
        }

        if (successCount > 0) {
            toast.success(`${successCount} arquivos vinculados ao patrimônio ${assetCode}!`);
            setLinkDialogOpen(false);
            setAssetCode("");
            loadDocuments();
        } else {
            toast.error("Falha ao vincular arquivos.");
        }
    };
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                    ...
                    <Button onClick={() => setScannerOpen(true)} className="gap-2">
                        <Scan className="w-4 h-4" /> Nova Digitalização
                    </Button>
                </div>
            </div>
                            ...
                            >
                                <CardContent className="p-4 flex flex-col gap-3 relative">
                                    <div className="absolute top-4 right-4 z-10">
                                        <Checkbox
                                            checked={isSelected}
                                            onCheckedChange={() => toggleSelection(doc.path)}
                                    ...
                                    <div className="h-40 bg-muted/50 rounded-md flex items-center justify-center group-hover:bg-background transition-colors">
                                        {isLinked ? (
                                            ...
                                            <FileText className="w-16 h-16 text-muted-foreground/40 group-hover:text-primary transition-colors" />
                                        )}
                                    </div>
                                    <div>
            ...
            <Dialog open={linkDialogOpen} onOpenChange={setLinkDialogOpen}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={handleLinkToAsset}>Salvar Vinculação</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
